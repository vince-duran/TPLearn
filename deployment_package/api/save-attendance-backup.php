...
try {
    ...
    // Get enrollments for this program to create sessions if needed
    $stmt = $conn->prepare("
    ...
    while ($row = $enrollments_result->fetch_assoc()) {
        $enrollments[$row['student_user_id']] = [
            ...
            'user_id_string' => $row['student_user_id_string']
        ];
    }
    
    // Get program time info for creating sessions
    $stmt = $conn->prepare("SELECT start_time, end_time FROM programs WHERE id = ?");
    $stmt->bind_param('i', $program_id);
    $stmt->execute();
    $program_info = $stmt->get_result()->fetch_assoc();
