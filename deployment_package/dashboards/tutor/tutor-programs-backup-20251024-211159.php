...
<body class="bg-gray-50 min-h-screen">
  ...
  <div class="flex">
    ...
    <div class="flex-1 lg:ml-64">
      ...
      renderTutorHeader('My Programs', getTutorCurrentDate());
      ?>
  ...
  <script>
    ...
    // CRITICAL: Define ALL essential functions immediately for onclick handlers
    ...
    window.viewProgramStream = function(programId) {
    ...
    window.manageAttendance = function(programId) {
    ...
    window.manageGrades = function(programId) {
    ...
    window.closeManageGradesModal = function() {
      const modal = document.getElementById('manageGradesModal');
    ...
    window.viewStudents = function(programId) {
    ...
    window.closeViewStudentsModal = function() {
      const modal = document.getElementById('viewStudentsModal');
      modal.classList.add('hidden');
      modal.classList.remove('flex');
    }
    ...
    window.emailStudent = function(email, name) {
    ...
    window.closeAttendanceManagementModal = function() {
      const modal = document.getElementById('attendanceManagementModal');
      modal.classList.add('hidden');
      modal.classList.remove('flex');
    }
        ...
        .then(data => {
          if (data.success && data.saved_sessions) {
            data.saved_sessions.forEach(sessionDate => {
              ...
              if (option && !option.textContent.includes('SAVED')) {
                option.textContent = `SAVED ${option.textContent}`;
    ...
    window.loadSessionAttendance = function(programId, sessionDate) {
      if (!sessionDate) {
    ...
    window.markAllAbsent = function() {
      TPAlert.confirm('Confirm Action', 'Mark all students as absent for this session?').then(result => {
        if (!result.isConfirmed) return;
    ...
    function sendAbsentNotices() {
      ...
      TPAlert.confirm('Confirm Action', `Send absence notices to ${absentStudents.length} student(s)?`).then(result => {
        if (!result.isConfirmed) return;
    ...
    window.saveAttendanceData = function() {
      ...
      .then(data => {
        ...
        if (data.success) {
          ...
          if (currentOption && !currentOption.textContent.includes('SAVED')) {
            currentOption.textContent = `SAVED ${currentOption.textContent}`;
